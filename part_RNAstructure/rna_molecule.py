...
class RNA_molecule:
    ...
    def __init__(self, sequence, name, err, label, path_to_rna_fold):
        ...
        self._energy = 0
	self._path_to_rna_fold = "RNAfold"
    ...
    def search_rna_struc(self, dict_db, path, label):
        ...
        try:
            ...
            if (which(self._path_to_rna_fold) == "None"):
            	addTextToLabel(label, "Could not find RNAfold executable in the given path: " + str(self._path_to_rna_fold) + "Please add a correct one.")
	    else:
            	####implement simons stuff
            	try:
            	    structure_pred = prediction.runRNAfold(self._sequence, self._path_to_rna_fold)
...
if __name__ == "__main__":
    ...
    mol = RNA_molecule("ACACGACGUAGCGUUAGACGUGACGUAGACGUAGAC", "HI-V", "test", label, "RNAfold")
